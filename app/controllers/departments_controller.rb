class DepartmentsController < ApplicationController
  ...
  uses_tiny_mce :only => [:new, :create, :edit, :update], 
                :options => {
                        ...
                        :plugins => %w{ table fullscreen safari spellchecker pagebreak style layer save advhr advimage advlink emotions iespell inlinepopups insertdatetime preview media searchreplace print contextmenu paste directionality noneditable visualchars nonbreaking xhtmlxtras template imagemanager filemanager },
  ...
  def update
    @department = Department.find(params[:id])
    params[:department][:accessible_menu_ids] ||= []
    respond_to do |format|
  ...
  def destroy
    @department = Department.find(params[:id])
    passed, msg = @department.verify_destroy
    if passed
      flash[:notice] = msg
    else
      flash[:error] = msg
    ...
    respond_to do |format|
      format.html { redirect_back_or_default(departments_url) }
      format.xml  { head :ok }
    end
  end
